...
class Searcher:
    ...
    def processPfVo(pf: vo.Frame, output_dir, unique_file_name=""):
        ...
        file_path = str(Path(f'{output_dir}\\_{int( time.time_ns() ) % 100000000}_.{args.img_format}'))
...
class PaddleFrame:
    ...
    def save(self):
        img_path = f"{self.output_path}\\{self.get_name()}.{args.img_format}"
            ...
            if nt_i >= 0:
                ...
                if max_score > args.th_sim_score:
                    ...
                    if cur_score > nt_score:
                        ...
                        # @MODIFY cur.get_name() = nt.get_name()
                        cur.id = nt.id
                        frame_list[nt_i] = cur
                        frame_list.pop( i )
            else:
...
class DelAnd2Pickle:
    ...
    def toPickle(self):
        ...
        self.o_path = utils.save_object_and_get_path( out_path=self.o_path_dir, o=self, name=self.name )
...
class Video( DelAnd2Pickle ):
    ...
    def __init__(self, video_path, output_dir, video_id, chapter_id, name=""):
        ...
        # 复制视频
        self.local_path = str(Path(output_dir).joinpath(Path(video_path).name)) # 视频存放的目录
        ...
        if self.fps > 0:  # 有些垃圾的视频文件, 没有内容, 导致 除0的报错
            ...
            self.kfs = [kf for s_kfs in self.sections_kfs for kf in s_kfs] # 合并各个分区的关键帧
            self.courseware_url =  self.__generate_courseware(CWPathType.URL) # 课件的URL的地址
    ...
    def __generate_courseware(self, path_type: CWPathType = CWPathType.LOCAL) -> str:
        ...
        if not self.courseware_path:
            ...
            self.courseware_path = utils.imgs2pdf(sorted_paths=imgs,output_dir=self.output_dir,file_name=self.name )
...
class Assember:
    ...
    def executeCourse(
        ...
        # 初始化 输出目录
        chapter_output_dir = output_dir + "\\" + dir_name
        # @modified 修改了路径
        chapter_output_dir = Assember._setDir( chapter_output_dir )
    ...
    def executeChapter(
        ...
        chapter_name = chapter_name or os.path.basename( chapter_dir_path )  # 获取目录名
